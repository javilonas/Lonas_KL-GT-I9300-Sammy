...
static int touchboost_switch_init(void)
{
	...
	if (sysfs_create_group(&touchboost_switch_control_device.this_device->kobj,
		...
		printk("Lonas_KL: failed to create touchboost switch sys fs object.\n");
		return 0;
	}

	// Print debug info
	printk("Lonas_KL: touchboost switch device initialized\n");
...
static void touchboost_switch_exit(void)
{
	...
	// Print debug info
	printk("Lonas_KL: touchboost switch device stopped\n");
}
