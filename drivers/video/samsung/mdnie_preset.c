					...
					const char *buf, size_t count)
{
	...
	if ((val >= 0) && (val <= 1))
	{
		...
		// write debug info
		if (mdnie_preset == 0)
			printk("Lonas-kernel: mdnie preset set to original\n");
		else
			printk("Lonas-kernel: mdnie preset set to Hardcore speedmod\n");
	}
...
static int mdnie_preset_init(void)
{
	...
	if (sysfs_create_group(&mdnie_preset_control_device.this_device->kobj,
		...
		printk("Lonas-kernel: failed to create sys fs object.\n");
		return 0;
	}

	// Print debug info
	printk("Lonas-kernel: mdnie control device started\n");
...
static void mdnie_preset_exit(void)
{
	...
	// Print debug info
	printk("Lonas-kernel: mdnie control device stopped\n");
}
